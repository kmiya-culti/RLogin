...
BOOL CFifoSocket::SocketLoop()
{
	...
	LONGLONG SendSize = 0;
	clock_t now = clock();
	clock_t RecvClock = now;
	clock_t SendClock = now;
	clock_t LastUpdate = now;
	DWORD RecvTimeOut = 0;
	DWORD SendTimeOut = 0;
	DWORD TimeOut;
	...
	while ( m_Threadtatus == FIFO_THREAD_EXEC ) {
		...
		if ( RecvTimeOut != 0 ) {
			...
			TimeOut = 3 * CLOCKS_PER_SEC;
		} else if ( m_TimeOut > 0 ) {
			now = clock();
			if ( (LastUpdate + (m_TimeOut * CLOCKS_PER_SEC)) <= now ) {
				m_nLastError = WAIT_TIMEOUT;
				return FALSE;
			} else {
				TimeOut = (DWORD)((LastUpdate + (m_TimeOut * CLOCKS_PER_SEC)) - now);
				bLastClock = TRUE;
			}
		} else
