...
void CFontTab::Init()
{
	...
	static const struct _FontInitTab	{
		...
		{ "JIS X 0201-Kana",		SET_94    | 'I',	'\x80', "JISX0201-1976",	SHIFTJIS_CHARSET,	100,	100,	0,	"ＭＳ ゴシック" },
		{ "JIS X 0201-Roman",		SET_94    | 'J',	'\x00', "JISX0201-1976",	SHIFTJIS_CHARSET,	100,	100,	0,	"ＭＳ ゴシック" },
		...
		{ "JIS X 0208-1978",		SET_94x94 | '@',	'\x00', "JIS_X0208-1983",	SHIFTJIS_CHARSET,	100,	100,	0,	"ＭＳ ゴシック" },
		{ "GB2312-1980",			SET_94x94 | 'A',	'\x00', "GB_2312-80",		GB2312_CHARSET,		100,	100,	0,	"" },
		{ "JIS X 0208-1983",		SET_94x94 | 'B',	'\x00', "JIS_X0208-1983",	SHIFTJIS_CHARSET,	100,	100,	0,	"ＭＳ ゴシック" },
		{ "KSC5601-1987",			SET_94x94 | 'C',	'\x00', "KS_C_5601-1987",	HANGEUL_CHARSET,	100,	100,	0,	"" },
		{ "JIS X 0212-1990",		SET_94x94 | 'D',	'\x00', "JIS_X0212-1990",	SHIFTJIS_CHARSET,	100,	100,	0,	"ＭＳ ゴシック" },
		{ "JIS X 0213-2000.1",		SET_94x94 | 'Q',	'\x00', "JIS_X0213-2000.1",	SHIFTJIS_CHARSET,	100,	100,	0,	"ＭＳ ゴシック" },
		{ "JIS X 0213-2000.2",		SET_94x94 | 'P',	'\x00', "JIS_X0213-2000.2",	SHIFTJIS_CHARSET,	100,	100,	0,	"ＭＳ ゴシック" },
...
void CTextRam::LineEditCwd(int ex, int sy, CStringW &cwd)
{
	...
	for ( x = 0 ; x < ex ; x += n ) {
		if ( x < (m_Cols - 1) && IS_1BYTE(vp[x].cm) && IS_2BYTE(vp[x + 1].cm) ) {
			//ch = m_IConv.IConvChar(m_FontTab[vp[x].md & CODE_MASK].m_IContName, "UTF-16BE", vp[x].ch);		// Char変換ではUTF-16BEを使用！
			...
			//ch = m_IConv.IConvChar(m_FontTab[vp[x].md & CODE_MASK].m_IContName, "UTF-16BE", vp[x].ch | m_FontTab[vp[x].md & CODE_MASK].m_Shift);		// Char変換ではUTF-16BEを使用！
			//ch = IconvToMsUnicode(ch);
			ch = vp[x].ch;
			if ( (ch & 0xFFFF0000) != 0 )	// 01020304 > 02 01 04 03 (BE->LE 2Word)
				str.PutWord(ch >> 16);
		...
		for ( x = (dm != 0 ? (x1 / 2) : x1) ; x <= x2 ; ) {
			...
			if ( x >= 0 && x < m_Cols ) {
				...
				if ( x < (m_Cols - 1) && IS_1BYTE(vp[0].cm) && IS_2BYTE(vp[1].cm) ) {
					//ch = m_IConv.IConvChar(m_FontTab[cm].m_IContName, "UTF-16BE", ch);			// Char変換ではUTF-16BEを使用！
					//ch = IconvToMsUnicode(ch);
					cn = 2;
				} else if ( !IS_ASCII(vp->cm) || ch == 0 ) {
					ch = 0;
					cm = (-1);
				//} else {
			...
			if ( pView->m_ClipFlag ) {
				pos = GetCalcPos((dm != 0 ? (x * 2) : x), y - pView->m_HisOfs + pView->m_HisMin);
			...
			if ( sa != ca || sf != cf || sb != cb || sm != cm || st != cn ) {
				...
				sm = cm;
				st = cn;
				rect.left = pView->CalcGrapX(x) * (dm ? 2 : 1);
			}
